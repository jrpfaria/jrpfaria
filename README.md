<h1 align="center">Hi, I'm Rodrigo Faria 👋</h1>

<p align="center">
  <i>Embedded / Firmware Engineer • FPGA & HLS Systems • Low-Level Performance Enthusiast</i>
</p>

---

### 👨‍💻 Profile

```text
jrpfaria@github
-------------------------
💻 C firmware developer for OLT devices
🎓 MSc in Computer Science — Universidade de Aveiro, Portugal
🧠 Focus: embedded systems, FPGA acceleration, and low-level optimization
🚀 Interests: performance tuning, hardware-aware design, compiler behavior
🎲 Enjoys: code golf, obfuscation, and pushing systems to their limits
````

---

### 🧠 Research & Engineering Work

#### 🎓 MSc Thesis — HLS-Oriented Automatic Code Generation for FPGA Acceleration of Neural Networks

**Focus:** Automatic HLS-based code generation for deploying neural networks on FPGAs at the edge.

* Designed a C-based code generation framework using HLS (High-Level Synthesis)
* Bridges high-level DNN descriptions → synthesizable FPGA implementations
* Evaluates RTL quality, latency, and resource utilization across architectures
* Compares HLS toolchains and optimization strategies for FPGA deployment flows
* Validated through FPGA inference experiments for real-time edge execution
* 🧩 Open-source implementation: https://github.com/jrpfaria/DNN2VHLS

---

#### 📄 Publications

<table>
<tr>
<td width="50%" valign="top">

**[1] [FPGA Acceleration of CNNs at the Edge: A Comparative Study on HLS Frameworks](https://doi.org/10.1109/NorCAS66540.2025.11231201)**

**Summary:**
Compares HDL Coder vs Vitis HLS for FPGA CNN deployment.
Highlights trade-offs between deterministic RTL mapping and flexible resource-efficient HLS synthesis for edge inference.

</td>

<td width="50%" valign="top">

**[2] [Using Automatic Code Generation for Real-Time FPGA Deployment of DNNs at the Edge](https://ieeexplore.ieee.org/document/11270598)**


**Summary:**
Presents an automatic HLS code generator for DNN deployment on FPGA.
Supports configurable CNN/DNN architectures enabling rapid prototyping and reproducible edge AI hardware workflows.

</td>
</tr>
</table>

---

<table>
<tr>

<td width="33%" valign="top">

#### ⚙️ Focus

- High-Level Synthesis (HLS)
- FPGA RTL optimization
- Embedded AI / edge inference
- Compiler-aware code generation
- Performance modeling & benchmarking

</td>

<td width="33%" valign="top">

#### 🧰 Stack

- **Languages:** C, C++, Java, Python, Bash, VHDL, MIPS Assembly

- **Tools:** Git, CUDA, OpenMP

- **Platform:** Linux (CachyOS / Arch)

</td>

<td width="33%" valign="top">

#### 🚩 Interests

- Embedded firmware systems
- Hardware-aware optimization
- Assembly & low-level reasoning
- Micro-optimizations
- Compiler internals

</td>

</tr>
</table>

---

## 📫 Contact

* GitHub: [https://github.com/jrpfaria](https://github.com/jrpfaria)
* LinkedIn: [https://www.linkedin.com/in/jrpfaria/](https://www.linkedin.com/in/jrpfaria/)
* Email: [jrfaria504@gmail.com](mailto:jrfaria504@gmail.com)
